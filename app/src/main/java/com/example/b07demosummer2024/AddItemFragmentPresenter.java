...
public class AddItemFragmentPresenter implements AddItemFragmentPresenterInterface {
   ...
   * @return str, if str does not match any string in stringList ignoring the case, otherwise the
   * ignored-case match of str in stringList
   */
  private String replaceStringWithListOccurence(List<String> stringList, String str) {
    for (String listStr : stringList) {
        if (listStr.equalsIgnoreCase(str)) {
            return listStr;
        }
    }
    return str;
  ...
  /**
   * validates the given item details and asks the model to add an item with given attributes to the
   * DB, if valid
   *
   ...
   */
  void addItem(String itemLotNumber, String itemName, String itemPeriod, String itemCategory,
      String itemDescription) {
    if (itemLotNumber.isEmpty() || itemName.isEmpty() || itemPeriod.isEmpty()
